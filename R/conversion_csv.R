...
csv.moodle <- function( fichier.csv,
    ...
    if ( any( is.null( colonne.temps ),
              ( is.integer( colonne.temps ) && ( colonne.temps < 1 ) ),
              ( is.character( colonne.temps ) && ( nchar( colonne.temps ) < 1 ) ) ) ) colonne.temps <- NA
...
{
    if ( nrow( d ) < 1 ) {
        stop( "Aucune question, conversion annul\u00e9." )
    }
    if ( ncol( d ) < 2 ) {
        stop( "Il faut au moins deux colonnes, \u00e9nonc\u00e9s et r\u00e9ponses." )
    }
    ...
    if ( all( any( c( "temps" ) %in% tolower( noms.colonnes ) ), 
              is.na( colonne.temps ) ) ) {
        colonne.temps <- which( tolower( noms.colonnes ) %in% c( "temps" ) )[ 1 ]
        cat( "  Colonne de temps d\u00e9tect\u00e9e  : colonne ", colonne.temps,
             " [", noms.colonnes[ colonne.temps ], "]\n", sep = "" )
    }

    ##  6) colonne des titres
    if ( all( any( c( "titre", "titres",
                      "title", "titles" ) %in% tolower( noms.colonnes ) ), 
              is.na( colonne.titre ) ) ) {
        colonne.titre <- which( tolower( noms.colonnes ) %in% c( "titre", "titres",
                                                                 "title", "titles" ) )[ 1 ]
        cat( "  Colonne de titres d\u00e9tect\u00e9e : colonne ", colonne.titre,
             " [", noms.colonnes[ colonne.titre ], "]\n", sep = "" )
    }
    ...
    for( q in codes ) {
        ...
        if ( nrow( dq ) == 1 ) {
            ...
            convertir.question_simple( question = dq,
                                       colonne.texte     = colonne.texte,
                                       colonne.reponse   = colonne.reponse,
                                       colonne.note      = colonne.note,
                                       colonne.note_question = colonne.note_question,
...
{
    if ( nrow( question ) != 1 ) {
        stop( "Une question simple doit correspondre \u00e0 une seule ligne." )
    }
    ...
    if ( any( is.na( reponse ), nchar( reponse ) < 1 ) ) {
        if ( !is.na( type.question ) ) {
            ...
            if ( type.question %in% c( 'compo', 'composition',
                ...
                if ( question$I.Categorie ) {
                    stop( "Question aussi identifi\u00e9e comme cat\u00e9gorie !" )
                }
        ...
        if ( !question$I.Categorie ) {
            stop( "Question non-identifi\u00e9e comme cat\u00e9gorie !" )
        }
    ...
    if ( question$I.Categorie ) {
        stop( "Question identifi\u00e9e \u00e0 tort comme cat\u00e9gorie !" )
    }
...
{
    if ( nrow( question ) < 2 ) {
        stop( "Il doit y avoir au moins deux lignes" )
    }
    ...
    if ( any( question$I.Categorie ) ) {
        stop( "Au moins une ligne faussement d\u00e9tect\u00e9e",
              " comme une cat\u00e9gorie." )
    }
    ...
    if ( FALSE == is.na( colonne.penalite ) ) {
        ...
        if ( length( penalite.question ) > 1 ) {
            warning( "Plusieurs p\u00e9nalit\u00e9s distinctes pour la m\u00eame question.",
                     " Seule la premi\u00e8re sera utilis\u00e*e." )
    ...
    if ( FALSE == is.na( colonne.temps ) ) {
        ...
        if ( length( temps ) > 1 ) {
            warning( "Plusieurs temps conseill\u00e9s pour la m\u00eame question.",
